...
*   **Display & Headlines (Space Grotesk):** Use for biometric IDs, system status, and large numerical data. Space Grotesk’s subtle "ink traps" provide a high-tech, engineered feel. 
    *   *Styling:* Set `headline-lg` to uppercase with a `0.05em` letter-spacing for a tactical look.
*   **Body & Labels (Inter):** Used for instructional text and metadata.
    *   *Styling:* `label-sm` should be used for "micro-data" (e.g., timestamp, coordinates) to maintain an editorial, information-dense aesthetic without sacrificing legibility.
